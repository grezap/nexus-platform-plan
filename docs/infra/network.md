...
- `dc-nexus` (192.168.70.240; canon `.10` per `vms.yaml`, pre-existing canon-vs-reality drift) runs Active Directory DNS for the `nexus.lab` forest. `dc-nexus-2` (192.168.70.242; canon `.11` — drift parallel to dc-nexus, see ADR-0039) is the foundation HA replica added in Phase 0.M, multi-master replication + replicated `nexus.lab` zone. Windows VMs join AD domain `nexus.lab`; DC Locator failover gives auth/DNS continuity on single-DC loss. **A replica DC MUST use static DNS** — `dc-nexus-2` is pinned to preferred `192.168.70.240` (the partner DC) + alternate `127.0.0.1` (self); if it reverts to DHCP DNS (the gateway `.1`, which does not serve `nexus.lab`) it cannot resolve the partner's `_msdcs.nexus.lab` CNAME and AD replication fails with **error 8524**, silently diverging while it is offline. Enforced in the foundation terraform (`role-overlay-dc-nexus-2-promotion.tf`, 2026-07-06); see `feedback_dc_nexus2_dhcp_dns_breaks_ad_replication`.
