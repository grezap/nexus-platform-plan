...
## Inventory — what the lab has built so far (140 VMs cold-rebuild proven across 11 tiers, through Phase 0.P)
...
| **08-citus** | `nexus-infra-citus` (`envs/citus`) | etcd DCS `citus-etcd-1/2/3` (3) · coordinator Patroni pair `citus-coord-1/2` (2, VIP `coord.citus.nexus.lab .211`) · worker-1 Patroni pair `citus-worker1-1/2` (2, VIP `.212`) · worker-2 Patroni pair `citus-worker2-1/2` (2, VIP `.213`) — 9 nodes + 3 leader-following VRRP VIPs; PG 17 + Citus 14.x | 0.P |
...
**Total: 140 VMs + 8 VRRP VIPs cold-rebuild proven (through Phase 0.P), reconciling with `docs/infra/vms.yaml` `vm_count: 140`. All sealed tiers tagged on their repos; observability tagged `nexus-infra-observability v0.1.0`; foundation HA partner `dc-nexus-2` (0.M, ADR-0039) ratified + cold-rebuild-proven 2026-05-28 (smoke-0.M 24/24); MongoDB sharded cluster (0.N, ADR-0040) + Vitess-sharded MySQL (0.O, `nexus-infra-vitess v0.1.0`, ADR-0041, smoke-0.O 71/71) + Citus-sharded PostgreSQL with full Patroni HA (0.P, `nexus-infra-citus v0.1.0`, ADR-0042, smoke-0.P 69/69) all live-ratified + cold-rebuild-proven. Future platform tools (0.J/0.K) are not yet built.**
...
| **The Citus-sharded PostgreSQL tier (0.P)** (3-node etcd DCS + coordinator Patroni pair + 2 worker Patroni pairs, each group fronted by a keepalived VRRP VIP that follows the Patroni leader; PG 17 + Citus 14.x; distributed + reference + colocated tables; full Vault-PKI mTLS, `clientcert=verify-ca`; ADR-0042) | [`nexus-infra-citus/docs/handbook.md` §1 — Phase 0.P walkthrough](https://github.com/grezap/nexus-infra-citus/blob/main/docs/handbook.md) | Foundation **alive** + security **alive** (env reads the 9 per-host AppRoles + `citus-server` PKI role + 4 KV creds from security at plan time) + the 2 citus templates built | ~25-35 min |
