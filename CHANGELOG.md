...
- **MASTER-PLAN.md** `0.P` row flipped to SEALED (topology + smoke 69/69 + fleet `131 → 140`); `I7` repo row marked SEALED `v0.1.0`. **`docs/infra/vms.yaml`** — citus cluster BUILT/SEALED; `vm_count: 140` (+9 citus). **`docs/infra/network.md`** — `08-citus` IPs (`.202`-`.213`) + 3 leader-following VRRP VIPs + the `:D7`-`:DF` MAC sub-table (high-water `DF`). **`docs/glossary.md`** — 7 new entries (Citus, coordinator, worker, distributed table, reference table, colocation). **`docs/demos/DEMO-22.md`** + System B `DEMO-27/28/29`. **ADR-0042** + index. 6 ratification transients chronicled in the handbook §3.x.
